...
public class OrbotHelper {
    ...
    public synchronized OrbotHelper addStatusCallback(Context c, StatusCallback cb) {
        if (statusCallbacks.size() == 0) {
          //AUDIT :: ADD NOT EXPORTED
          ContextCompat.registerReceiver(c.getApplicationContext(), orbotStatusReceiver, new IntentFilter(OrbotHelper.ACTION_STATUS), ContextCompat.RECEIVER_NOT_EXPORTED);
