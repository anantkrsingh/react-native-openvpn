...
import com.openvpn.NotificationBundle;
import com.openvpn.R;
...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    private static final String RESUME_VPN = "com.openvpn.RESUME_VPN";
    ...
    //AUDIT
    private Bundle NotificationData = new Bundle();
    ...
    private void endVpnService() {
        ...
        if (!mStarting) {
            ...
            if (!mNotificationAlwaysVisible) {
                stopSelf();
              //AUDIT, DO NOT UNREGISTER LISTENR, CAUSE NOTIFICATION NOT SHOW UP
...
//            nbuilder.setContentTitle(getString(R.string.notifcation_title_notconnect));
        nbuilder.setContentTitle(NotificationData.getString(NotificationBundle.TITLE_NOTIFICATION));


        //AUDIT
        ConnectionStatus currentState = VpnStatus.getStatus();
        if(currentState == LEVEL_CONNECTED){
          String localTitle = NotificationData.getString(NotificationBundle.TITLE_CONNECTED);
          if(localTitle!=null && !localTitle.isEmpty()){
            nbuilder.setContentText(localTitle);
          }else{
            nbuilder.setContentText(msg);
          }
        }else if(currentState==LEVEL_AUTH_FAILED || currentState == LEVEL_NONETWORK ||currentState==LEVEL_NOTCONNECTED){
          //DISCONNECTED
          String localTitle = NotificationData.getString(NotificationBundle.TITLE_DISCONNECTED);
          if(localTitle!=null && !localTitle.isEmpty()){
            nbuilder.setContentText(localTitle);
          }else{
            nbuilder.setContentText(msg);
          }
        }else if(currentState==LEVEL_VPNPAUSED){
          //paused
          String localTitle = NotificationData.getString(NotificationBundle.TITLE_PAUSED);
          if(localTitle!=null && !localTitle.isEmpty()){
            nbuilder.setContentText(localTitle);
          }else{
            nbuilder.setContentText(msg);
          ...
          //connecting
          String localTitle = NotificationData.getString(NotificationBundle.TITLE_CONNECTING);
          if(localTitle!=null && !localTitle.isEmpty()){
            nbuilder.setContentText(localTitle);
          }else{
            nbuilder.setContentText(msg);
        ...
        //AUDIT
        if(NotificationData.getBoolean(NotificationBundle.SHOW_TIMER)){
          ...
          //AUDIT
        try{
          Intent opener = new Intent(this, Class.forName(Objects.requireNonNull(NotificationData.getString(NotificationBundle.ACTIVITY_PACKAGE_NAME))));
          PendingIntent opIntent = PendingIntent.getActivity(this, 0, opener, PendingIntent.FLAG_IMMUTABLE);
          nbuilder.setContentIntent(opIntent);
        } catch (Exception e) {
...
//                setUsesChronometer.invoke(nbuilder, true);
    ...
    private void addVpnActionsToNotification(Notification.Builder nbuilder) {
        ...
        if(NotificationData.getBoolean(NotificationBundle.SHOW_DISCONNECT)){
          if(!Objects.requireNonNull(NotificationData.getString(NotificationBundle.TITLE_ACTION_DISCONNECT)).isEmpty()){
        ...
        if(false && NotificationData.getBoolean(NotificationBundle.SHOW_PAUSE)){
          if (mDeviceStateReceiver == null || !mDeviceStateReceiver.isUserPaused()) {
              pauseVPN.setAction(PAUSE_VPN);
              PendingIntent pauseVPNPending = PendingIntent.getService(this, 0, pauseVPN, PendingIntent.FLAG_IMMUTABLE);
              //AUDIT
              if(!Objects.requireNonNull(NotificationData.getString(NotificationBundle.TITLE_ACTION_PAUSE)).isEmpty()){
                nbuilder.addAction(R.drawable.ic_menu_pause,
                  NotificationData.getString(NotificationBundle.TITLE_ACTION_PAUSE), pauseVPNPending);
              }else{
                nbuilder.addAction(R.drawable.ic_menu_pause,
              ...
              //AUDIT
              if(!Objects.requireNonNull(NotificationData.getString(NotificationBundle.TITLE_ACTION_RESUME)).isEmpty()) {
                nbuilder.addAction(R.drawable.ic_menu_play,
                  NotificationData.getString(NotificationBundle.TITLE_ACTION_RESUME), resumeVPNPending);
              }else{
                nbuilder.addAction(R.drawable.ic_menu_play,
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if(intent!=null){
          NotificationData = intent.getBundleExtra(NotificationBundle.BUNDLE_KEY);
        }
